    ...
    stages {
        stage('Git checkout') {
            steps {
                ...
                git branch: 'main', url: 'https://github.com/Tarshi-Y/devops-fully-automated.git/'
        ...
        stage('SonarQube scanning') {
            steps {
                withSonarQubeEnv('SonarQube') {
                    withCredentials([string(credentialsId: 'sonarqube-token', variable: 'AYVMiVrfpP1zKgYnwIzs')]) {
                        sh """
                   mvn sonar:sonar \
                 -Dsonar.projectKey=JavaProject \
                 -Dsonar.host.url=http://172.31.80.80:9000 \
                 -Dsonar.login=efcc9eb918173f4cea20288e37137e1b7625a4de
                    """
                    }
                }
            }
